...
export const displayRatingStar = (reviewList) => {
  // Kiểm tra reviewList có tồn tại và có data không
  if (!reviewList || !reviewList.data) {
    return 0;
  }

  // Kiểm tra data có phải là array và có phần tử không  
  if (!Array.isArray(reviewList.data) || reviewList.data.length === 0) {
    return 0;
  }
  ...
  try {
    ...
    const totalCount = ratingArray.reduce((sum, count) => sum + count, 0);
    if (totalCount === 0) return 0;

    const sum = ratingArray.reduce((sum, count, index) => sum + count * (index + 1), 0);
